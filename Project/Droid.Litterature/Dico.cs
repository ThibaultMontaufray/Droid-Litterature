...
namespace Droid.Litterature
{
    ...
    public class Dico
    {
        ...
        private List<Syllabe> DEBUG_InsertNewSyllabe(List<Syllabe> ls)
        {
            ...
            {
                DBAdapter.ExecuteQuery(Parameters.Config["DB_NAME"].ToString(), string.Format("insert into {0}.s_unknownsyllabe (valeur) values ('{1}')", Parameters.Config["DB_SCHEMA"].ToString(), item.Text));   
            }
            return null;
        }
        #endregion
